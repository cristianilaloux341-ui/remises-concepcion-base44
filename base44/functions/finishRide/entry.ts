...
Deno.serve(async (req) => {
  ...
  const checkAndRepairDriver = async (currentDriver) => {
    ...
    const fixRes = await b44.entities.Driver.updateMany(
      ...
      { $set: { status: 'disponible', dispatch_status: 'normal',  queue_entered_at: null, queue_authoritative_base: null, queue_position: null, reserved_order_id: null, reservation_token: null, driver_reservation_key: null, active_ride_id: null } }
    );
