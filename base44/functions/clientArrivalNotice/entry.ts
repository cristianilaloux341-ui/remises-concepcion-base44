...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!orderId || !driverId || !sessionToken) return Response.json({ success:false, reason:'missing_fields' }, { status:400 });
    ...
    if (count === 1 && firstAt && now - firstAt < TWO_MINUTES) return Response.json({ success:false, reason:'second_notice_too_early', retryAt:new Date(firstAt + TWO_MINUTES).toISOString() }, { status:409 });

    const sentAt = new Date(now).toISOString();
    const patch:any = { status:'en_camino', client_arrival_notice_count:count + 1, client_arrival_expires_at:null };
    if (count === 0) {
      ...
      // Segundo aviso manual. No inicia temporizador ni cancela el viaje.
      // Si el cliente no aparece, la central decide manualmente que hacer.
    ...
    if (order.client_id) {
      try {
        const response = await base44.functions.invoke('sendClientPush', { clientId:order.client_id, orderId:order.id, driverId, sessionToken, noticeNumber:count + 1, type:'arrival' });
        pushResult = response?.data || response;
      } catch (e:any) { pushResult = { success:false, reason:e?.message || 'push_error' }; }
    }
    ...
    return Response.json({ success:true, noticeNumber:count + 1, sentAt, expiresAt:null, manualCancellationRequired:count >= 1, pushSent:pushResult?.success === true, pushReason:pushResult?.reason || null });
  } catch (e:any) {
