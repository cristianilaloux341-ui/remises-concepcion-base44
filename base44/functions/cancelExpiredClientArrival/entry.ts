...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Idempotencia: una ejecución tardía nunca debe volver a liberar ni alterar otro viaje.
    if (order.status === 'cancelado' && order.client_arrival_cancel_reason === 'cliente_no_responde') {
    ...
    if (Number(order.client_arrival_notice_count || 0) < 2 || !order.client_arrival_second_notice_at || !order.client_arrival_expires_at) {
      return Response.json({ success:false, reason:'second_notice_not_completed' }, { status:409 });
    }
    const expiresAt = Date.parse(order.client_arrival_expires_at);
    if (!Number.isFinite(expiresAt) || Date.now() < expiresAt) {
      return Response.json({ success:false, reason:'not_expired', expiresAt:order.client_arrival_expires_at }, { status:409 });
    }

    // Última lectura justo antes de cancelar: protege contra YA VOY llegando al límite.
    order = await b44.entities.RideOrder.get(orderId).catch(() => null);
    if (!order || order.status !== 'en_camino' || order.client_arrival_acknowledged === true) {
      return Response.json({ success:false, reason:'state_changed_before_cancel' }, { status:409 });
    }
    ...
    await b44.entities.RideOrder.update(orderId, {
      status:'cancelado',
      client_arrival_cancel_reason:'cliente_no_responde',
      client_arrival_expires_at:null,
    ...
    if (driver) {
      // No tocar referencias que ya hayan cambiado a otro viaje.
      const patch:any = {};
      ...
      if (stillOwnsThisRide) {
        ...
        patch.dispatch_status = 'normal';
        patch.reservation_token = null;
        patch.manual_reservation_token = null;
        patch.driver_reservation_key = null;

        if (driver.current_base) {
          // FIFO usa queue_entered_at. Una fecha suficientemente anterior lo devuelve primero sin tocar a los demás.
          queueEnteredAt = new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString();
          patch.queue_entered_at = queueEnteredAt;
          patch.queue_position = 1;
          returnedFirst = true;
        }
    ...
    await b44.entities.AuditLog.create({
      action:'CLIENT_NO_RESPONSE_AUTO_CANCEL',
      user_type:'sistema',
      user_name:'Sistema',
