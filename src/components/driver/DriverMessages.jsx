...
export default function DriverMessages({ driver, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-white flex flex-col z-[9999]" style={{ paddingBottom: 'env(safe-area-inset-bottom)', paddingTop: 'env(safe-area-inset-top)' }}>
      {/* Header */}
          ...
          return (
            <div key={msg.id} className={`flex ${isFromMe ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 ${
                isFromMe ? "bg-slate-900 text-white rounded-br-sm" : "bg-white text-slate-900 border border-gray-200 rounded-bl-sm"
              }`}>
                {!isFromMe && (
